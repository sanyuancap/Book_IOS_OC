...
{
    char *sql = "CREATE TABLE member(id integer  primary key autoincrement , name text, age integer, addr text, sex text)";
...
- (NSMutableArray*) getAllDatas{
    ...
    while (sqlite3_step(statement) == SQLITE_ROW)
    {
        ...
        if (name)
        {
            [currRow setObject:[NSString stringWithCString:name] forKey:@"name"];
        }
        
        int age       = sqlite3_column_int(statement, 2);
        [currRow setObject:[NSString stringWithFormat:@"%d",age] forKey:@"age"];
        
        char* addr     = (char*)sqlite3_column_text(statement, 3);
        if (addr)
        {
           [currRow setObject:[NSString stringWithCString:addr] forKey:@"addr"];
        }
        
        char* sex     = (char*)sqlite3_column_text(statement, 4);
        if (sex)
        {
            [currRow setObject:[NSString stringWithCString:sex] forKey:@"sex"];
        }
...
- (NSMutableArray*) getStudentsByName:(NSString*)tname
{
    NSMutableArray* allDatas = [[[NSMutableArray alloc] init] autorelease];
    sqlite3_stmt *statement = nil;
    char *sql = "SELECT * FROM member where name clob '%?%'";
    ...
    sqlite3_bind_text(statement, 1, [tname UTF8String], -1, SQLITE_TRANSIENT);
